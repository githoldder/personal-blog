...
5. Active PRD JSON: `prds/json/S04-lab.json`
...
当前 Sprint 目标：在已有内容管线之上实现可解释的语义图谱实验区，让知识资产之间的关系可以被生成、检查和交互式浏览。

S01 Foundation 已完成骨架搭建。S02 Sense v4.0 Harness Adoption 已完成治理硬化。S03 Asset Build Pipeline 已完成内容真源到多端输出闭环。S04 进入 Semantic Lab 阶段，但仍必须遵守 Sense 权限、PRD 双板和外部副作用审计规则。

**不做的事情（禁止碰区）：**
- 未经 S04-T04 可行性决策的 3D 语义图谱实现
- 未经 S04-T05 输入策略确认的 MediaPipe 手势识别
- 未经 S04-T05 输入策略确认的 WebGPU 语义计算
