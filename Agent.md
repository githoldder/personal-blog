...
5. Active PRD JSON: `prds/json/S03-pipeline.json`
6. Relevant `.agent/rules/`, `.agent/workflows/`, `.agent/skills/`

## 当前阶段

**S03 — Asset Build Pipeline**

当前 Sprint 目标：把内容真源到多端输出的核心构建管线做成可用闭环，使简历、演示文稿、笔记和项目都能从 `content/` 稳定生成 Web 与静态资产。

S01 Foundation 已完成骨架搭建。S02 Sense v4.0 Harness Adoption 已完成治理硬化。S03 进入 Pipeline 阶段，但仍必须遵守 Sense 权限、PRD 双板和外部副作用审计规则。

**不做的事情（禁止碰区）：**
- 3D 语义图谱渲染（Three.js / WebGL）
- MediaPipe 手势识别
- WebGPU 语义计算
...
1. **S03-T01**: Make resume YAML validation explicit
2. **S03-T02**: Generate resume web and PDF artifacts
3. **S03-T03**: Define deck metadata and build manifest
4. **S03-T04**: Make deck outputs browsable
5. **S03-T05**: Promote notes and projects to typed content collections
